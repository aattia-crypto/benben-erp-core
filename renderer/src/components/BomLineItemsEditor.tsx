...
import { newLineItem, type LineItemRow } from "@/components/LineItemsEditor";

type BomLineItemsEditorProps = {
  lines: LineItemRow[];
  onChange: (lines: LineItemRow[]) => void;
};

/** BOM component lines — SKU picked from live inventory; material/UOM/cost auto-filled. */
export function BomLineItemsEditor({ lines, onChange }: BomLineItemsEditorProps) {
  const { items } = useProductCatalog();
  const activeItems = items.filter((i) => i.status === "active");

  function patchLine(id: string, updates: Partial<LineItemRow>) {
  ...
  function onSkuSelect(lineId: string, sku: string) {
    ...
    if (!item) {
      patchLine(lineId, { sku: "", description: "", uom: "ea", unitCost: 0 });
      return;
    }
    patchLine(lineId, {
      sku: item.sku,
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <ErpFieldLabel>Component lines</ErpFieldLabel>
        <button
          type="button"
      ...
      {activeItems.length === 0 ? (
        <p className="rounded-md border border-dashed border-border bg-surface px-3 py-2 text-xs text-muted-foreground">
          Add inventory items first (Inventory module) before defining BOM components.
        </p>
      ) : null}
            ...
            {lines.map((l) => (
              <tr key={l.id} className="border-t border-border">
                ...
                <td className="px-2 py-1">
                  <input
                    type="number"
                    className={`${erp.input} bg-erp-readonly text-right ${erp.financial}`}
                    value={l.unitCost}
                    readOnly
                    tabIndex={-1}
                  />
