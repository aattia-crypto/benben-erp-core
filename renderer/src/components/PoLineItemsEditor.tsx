...
/** PO line grid — SKU from inventory; description/UOM auto-filled; qty and unit cost editable. */
    ...
    <div className="space-y-2">
      ...
      {activeItems.length === 0 ? (
        <p className="rounded-md border border-dashed border-border bg-surface px-3 py-2 text-xs text-muted-foreground">
          Add inventory items first (Inventory module) before creating purchase order lines.
        </p>
      ) : null}
              ...
              <tr key={l.id} className="border-t border-border">
                ...
                <td className="px-2 py-1">
                  <input
                    ...
                    value={l.unitCost}
                    onChange={(e) => patchLine(l.id, { unitCost: Number(e.target.value) })}
                  />
                </td>
